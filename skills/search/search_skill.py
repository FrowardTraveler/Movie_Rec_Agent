...
class SearchSkill(BaseSkill):
    ...
    async def _execute(self, query: str, user_id: str = None, top_k: int = 5) -> Dict[str, Any]:
        """
        执行搜索
    ...
    async def _search_local(self, query: str, top_k: int) -> List[Dict[str, Any]]:
        ...
        movies_file = os.path.join(
            os.path.dirname(__file__), "..", "..", "data", "movies_rich.json"
        )
        ...
        for movie in movies:
            ...
            genres = (
                movie.get("genres", "").lower()
                if isinstance(movie.get("genres"), str)
                else " ".join(movie.get("genres", [])).lower()
            )
