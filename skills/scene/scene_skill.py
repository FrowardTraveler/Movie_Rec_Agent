...
class SceneSkill(BaseSkill):
    ...
    scene_recommendations = {
        "date": {
            "genres": ["爱情", "浪漫"],
            "response": "约会适合看浪漫温馨的爱情电影~"
        },
        "alone": {
            "genres": ["剧情", "科幻", "悬疑"],
            "response": "独处时适合看一些有深度的电影~"
        },
        "party": {
            "genres": ["喜剧", "动作"],
            "response": "聚会适合看轻松欢乐的电影~"
        },
        "relax": {
            "genres": ["喜剧", "治愈"],
            "response": "放松时适合看轻松治愈的电影~"
        },
        "learn": {
            "genres": ["纪录片", "剧情"],
            "response": "学习时适合看有教育意义的电影~"
        },
    }
    ...
    async def _execute(
        ...
        if not matched_scene:
            return {
                "success": True,
                "response": f"你可以告诉我你现在的场景，比如：约会、独处、聚会、放松、学习，我会为你推荐合适的电影~",
                "skill": self.name
            }
