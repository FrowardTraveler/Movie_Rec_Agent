...
class MockRedis:
    ...
    async def xdel(self, stream_name, msg_id):
        if stream_name in self._streams:
            self._streams[stream_name] = [
                (m_id, fields) for m_id, fields in self._streams[stream_name] if m_id != msg_id
            ]
...
async def test_enqueue_task(task_queue):
    ...
    result = await task_queue.enqueue(
        task_type="test_task", payload={"user_id": "test", "data": "value"}
    )
...
async def test_process_message(task_queue):
    ...
    fields = {"type": "test_task", "payload": '{"user_id": "test", "value": "hello"}'}
...
async def test_process_message_unknown_type(task_queue):
    """测试未知任务类型"""
    fields = {"type": "unknown_task", "payload": "{}"}
...
async def test_process_message_handler_error(task_queue):
    ...
    fields = {"type": "failing_task", "payload": "{}"}
...
async def test_enqueue_with_priority(task_queue):
    """测试带优先级的任务入队"""
    result = await task_queue.enqueue(
        task_type="high_priority_task", payload={"data": "important"}, priority=10
    )
    assert result is True
