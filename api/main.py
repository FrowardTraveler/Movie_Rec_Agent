...
from services.auth.jwt_auth import AuthUser, verify_token
from services.cache.redis_cache import redis_cache
from services.metrics import prometheus, record_http_request
...
async def rate_limit_middleware(request: Request, call_next):
    ...
    if not allowed:
        ...
        from api.exceptions import ErrorCode, _build_error_response
...
async def metrics():
    ...
    return Response(content=prometheus.get_metrics(), media_type="text/plain; charset=utf-8")


@app.post("/api/recommend", response_model=RecommendResponse)
async def recommend(
    request: RecommendRequest, user: Optional[AuthUser] = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        # A/B 测试：分配推荐策略
        variant = (
            ab_test_service.assign_variant("recommend_strategy", effective_user_id) or "llm_only"
        )
        ...
        if variant == "llm_only":
            ...
            result = await multi_agent_coordinator.process_request(
                user_input=request.query, user_id=effective_user_id
            )
    ...
    async def generate_stream() -> AsyncGenerator[str, None]:
        try:
            ...
            error_data = json.dumps(
                {"type": "error", "message": f"系统错误: {str(e)}"}, ensure_ascii=False
            )
...
async def get_dialogue_history(
    user_id: str, n: int = 10, user: Optional[AuthUser] = Depends(get_current_user)
):
    """
    获取对话历史
...
async def update_user_profile(
    user_id: str, preferences: dict, user: Optional[AuthUser] = Depends(get_current_user)
):
    """
    更新用户偏好
    ...
    result = await profile_skill.execute(user_id=user_id, action="update", preferences=preferences)
...
async def submit_feedback(
    request: FeedbackRequest, user: Optional[AuthUser] = Depends(get_current_user)
):
    """
    提交推荐反馈
...
async def get_feedback_history(
    user_id: str, limit: int = 20, user: Optional[AuthUser] = Depends(get_current_user)
):
    """获取用户反馈历史"""
    from services.feedback.feedback_service import feedback_service
...
async def get_user_profile_stats(
    user_id: str, user: Optional[AuthUser] = Depends(get_current_user)
):
    """获取用户画像统计"""
    from services.feedback.feedback_service import feedback_service
...
class UserEventRequest(BaseModel):
    ...
    event_type: str = Field(
        ..., description="事件类型: impression/click/detail_view/rating/like/dislike"
    )
...
async def record_user_event(
    request: UserEventRequest, user: Optional[AuthUser] = Depends(get_current_user)
):
    """
    记录用户行为事件
...
async def get_evaluation_metrics(
    ...
    if strategy == "all":
        ...
        traditional_metrics = await online_metrics_service.calculate_metrics(
            "traditional", time_window
        )
