...
class MasterAgent(BaseAgent):
    ...
    async def execute(self, state: MultiAgentState) -> MultiAgentState:
        ...
        try:
            ...
            task_plan_without_summary = [
                t for t in state["task_plan"] if t.get("agent") != "summary"
            ]
    ...
    async def _create_task_plan(self, state: MultiAgentState) -> MultiAgentState:
        ...
        intent_analysis = await self._analyze_intent_with_llm(user_input, recent_history)
...
注意：只返回 JSON，不要其他内容。"""
    ...
    def _inherit_context_entities(
        self, recent_history: list, current_entities: Dict[str, Any], intent_type: str
    ) -> Dict[str, Any]:
        """
        从历史对话中继承实体
        ...
        is_ambiguous = any(
            phrase in current_entities.get("other", "") for phrase in ambiguous_phrases
        )
    ...
    async def _execute_task_isolated(self, state: MultiAgentState, task: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "agent_results": [
                    {"agent_name": agent_name, "success": False, "data": None, "error": str(e)}
                ]
            }
