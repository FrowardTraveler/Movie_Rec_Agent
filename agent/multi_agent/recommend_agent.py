...
class RecommendAgent(BaseAgent):
    ...
    async def execute(self, state: MultiAgentState) -> MultiAgentState:
        ...
        try:
            ...
            if not llm_result["success"] or not llm_result["data"] or not llm_result["data"].get("items"):
            ...
            for item in llm_items:
                logger.info(f"      - {item.get('title_zh', item.get('title', 'unknown'))} ({item.get('title_en', 'N/A')})")
            ...
            if verified_items:
                logger.info(f"   [验证结果] {len(verified_items)}/{len(llm_items)} 部电影在数据库中找到")
    ...
    async def _generate_with_llm(self, state: MultiAgentState, extracted_info: Dict[str, Any]) -> Dict[str, Any]:
        """
        LLM 生成推荐（自由生成）
        """
...
注意：只返回 JSON，不要其他内容。"""
        ...
        if already_recommended:
            user_prompt += f"【已推荐列表】（绝对不要重复推荐这些电影）：\n"
    ...
    async def _verify_with_kb(self, llm_items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for item in llm_items:
            ...
            if movie_info:
                ...
                logger.info(f"   [验证✓] '{title_zh}' ({title_en}) -> '{found_title_en}' ID:{movie_id}")
            else:
    ...
    def _extract_recommended_movies(self, memory_context: str) -> List[str]:
        ...
        for line in memory_context.split("\n"):
            ...
            if line.startswith("- ") and ("上映" in line or "年" in line or "评分" in line or "(" in line):
                # 提取电影名（在 "- " 和 " (" 之间）
            ...
            # 匹配逗号分隔的电影名列表行
                ...
                for part in parts:
                    part = part.strip()
                    if part and len(part) > 1 and not part.startswith("【") and not part.startswith("-"):
                        movies.append(part)
