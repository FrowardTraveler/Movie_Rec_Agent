            ...
            try:
                ...
                if result.get("success"):
                    ...
                    if items:
                        movies_str = "\n".join(
                            [
                                f"- {m.get('title', '未知')} ({m.get('genres', '')}) 评分: {m.get('rating', 'N/A')}"
                                for m in items[:5]
                            ]
                        )
...
特别注意：如果用户说"心情不好"、"无聊"、"想看电影"等，一定要调用 recommend_movies 工具！"""
    ...
    def _save_conversation(self, user_id: str, user_msg: str, assistant_msg: str):
        ...
        self._conversation_memory[user_id].append({"user": user_msg, "assistant": assistant_msg})

        if len(self._conversation_memory[user_id]) > 10:
            self._conversation_memory[user_id] = self._conversation_memory[user_id][-10:]

    async def invoke(
        self, user_input: str, user_id: str = "anonymous", messages: Sequence[BaseMessage] = None
    ) -> Dict[str, Any]:
        """调用 Agent 处理用户请求"""
        ...
        try:
            ...
            elapsed = time.time() - start_time
            logger.error(
                "Agent 执行失败", user_id=user_id, error=str(e), latency_ms=round(elapsed * 1000, 2)
            )
