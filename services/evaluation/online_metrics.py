...
class OnlineMetricsService:
    ...
    async def calculate_metrics(
        ...
        try:
            ...
            rating_participation_rate = total_ratings / total_impressions if total_impressions > 0 else 0.0
            ...
            satisfaction_index = self._calculate_satisfaction_index(
                total_likes, total_dislikes, total_ratings, total_impressions,
    ...
    async def get_ab_comparison(
        ...
        if llm_metrics and traditional_metrics:
            comparison["comparison"] = {
                ...
                "avg_rating_diff": round(llm_metrics.get("avg_rating", 0) - traditional_metrics.get("avg_rating", 0), 2),
                "hit_rate_at_10_diff": round(
                    llm_metrics.get("hit_rate_at_10", 0) - traditional_metrics.get("hit_rate_at_10", 0), 4
                ),
                "satisfaction_diff": round(
                    llm_metrics.get("satisfaction_index", 0) - traditional_metrics.get("satisfaction_index", 0), 4
                ),
                "diversity_diff": round(
                    llm_metrics.get("genre_diversity", 0) - traditional_metrics.get("genre_diversity", 0), 4
                ),
            }
