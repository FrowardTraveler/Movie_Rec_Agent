    ...
    def assign_variant(self, experiment_id: str, user_id: str) -> Optional[str]:
        ...
        for variant_name, weight in experiment.variants.items():
            ...
            if normalized < cumulative:
                ...
                logger.info("A/B 测试分配变体", experiment_id=experiment_id, user_id=user_id, variant=variant_name)
                return variant_name
        
        # 兜底返回最后一个
        return list(experiment.variants.keys())[-1]
    
    async def record_result(self, experiment_id: str, user_id: str, variant: str, result: Dict[str, Any]):
        """
        记录实验结果
