...
class FeedbackService:
    ...
    async def submit_feedback(self, feedback: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            pipe.rpush(
                f"{self.FEEDBACK_KEY_PREFIX}all", json.dumps(feedback_data, ensure_ascii=False)
            )
            pipe.rpush(
                f"{self.FEEDBACK_KEY_PREFIX}user:{feedback_data['user_id']}",
            ...
            await self._update_user_profile(
                feedback_data["user_id"], feedback_data["rated_movie_id"], feedback_data["rating"]
            )

            logger.info(
                "反馈已提交", user_id=feedback_data["user_id"], rating=feedback_data["rating"]
            )
            return True
    ...
    async def get_user_feedback(self, user_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            data = await self._redis_client.lrange(
                f"{self.FEEDBACK_KEY_PREFIX}user:{user_id}", -limit, -1
            )
