...
class TraditionalRecommendationClient:
    ...
    async def initialize(self):
        ...
        if not self._initialized:
            ...
            logger.info("传统推荐 API 客户端初始化完成", base_url=self.base_url, timeout=_HTTP_TOTAL_TIMEOUT)
    ...
    async def resolve_llm_movie_ids(self, llm_items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for item in llm_items:
            ...
            if movie_info:
                ...
                logger.info("找到电影", original=title, resolved=found_title, movie_id=movie_info.get("movie_id"))
            else:
