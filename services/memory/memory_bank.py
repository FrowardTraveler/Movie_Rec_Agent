...
class MemoryBank:
        ...
        try:
            ...
            if data:
                ...
                for category, items in raw.items():
                    self._memory[user_id][category] = [MemoryItem.from_dict(item) for item in items]
    ...
    def _clean_expired(self, user_id: str):
        ...
        for category in list(self._memory[user_id].keys()):
            self._memory[user_id][category] = [
                item for item in self._memory[user_id][category] if now - item.timestamp < item.ttl
            ]
            if not self._memory[user_id][category]:
                del self._memory[user_id][category]

    async def add(
        self, user_id: str, key: str, value: Any, category: str = "context", ttl: int = 3600
    ):
        """
        添加记忆
        ...
        # 移除同名旧记忆
        self._memory[user_id][category] = [
            item for item in self._memory[user_id][category] if item.key != key
        ]

        item = MemoryItem(key=key, value=value, category=category, timestamp=time.time(), ttl=ttl)
    ...
    async def get_recent(self, user_id: str, category: str, limit: int = 1) -> List[Any]:
        ...
        items = sorted(self._memory[user_id][category], key=lambda x: x.timestamp, reverse=True)
    ...
    async def save_recommendation(
        self, user_id: str, movies: List[Dict], query: str, ttl: int = 1800
    ):
        """
        保存推荐结果到记忆
